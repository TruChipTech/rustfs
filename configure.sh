...
usage() {
    ...
    echo "  allyesconfig   - Generate a complete config from Kconfig (every applet/feature)"
...
cmd_defconfig() {
    ...
    # Warn if the static defconfig has drifted behind Kconfig (e.g. applets
    # were added to Kconfig/src but never enabled in configs/default_defconfig).
    local missing
    missing=$(comm -23 \
        <(grep -E '^config APPLET_' "Kconfig" | awk '{print $2}' | sort -u) \
        <(grep -oE '^CONFIG_APPLET_[A-Z0-9_]+' "$DOTCONFIG" | sed 's/^CONFIG_//' | sort -u))
    if [ -n "$missing" ]; then
        echo "WARNING: default_defconfig is missing these Kconfig applets:" >&2
        echo "$missing" | tr '\n' ' ' >&2; echo >&2
        echo "         Run '$0 allyesconfig' for a complete config." >&2
    fi
}

# Generate a complete .config directly from Kconfig: enable every applet and
# every boolean feature. Cannot drift because it is derived from Kconfig itself.
cmd_allyesconfig() {
    {
        echo "# Generated by '$0 allyesconfig' — all features enabled, static build"
        echo "CONFIG_STATIC=y"
        # Enable every symbol except the mutually-exclusive init choice, which is
        # forced to a single implementation (rustfs) below.
        grep -E '^config [A-Z0-9_]+' "Kconfig" | awk '{print "CONFIG_"$2"=y"}' \
            | grep -vE '^CONFIG_INIT_(RUSTFS|SYSVINIT|SYSTEMD)=y$' | sort -u
        echo "CONFIG_INIT_RUSTFS=y"
        echo "# CONFIG_INIT_SYSVINIT is not set"
        echo "# CONFIG_INIT_SYSTEMD is not set"
    } > "$DOTCONFIG"
    echo "Generated all-features configuration -> $DOTCONFIG"
    echo "Applets enabled: $(grep -cE '^CONFIG_APPLET_.*=y' "$DOTCONFIG")"
    echo "Init system: rustfs (use menuconfig to choose sysvinit/systemd)"
}
